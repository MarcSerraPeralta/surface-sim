...
def qec_circuit(func: LogOpCallable) -> LogOpCallable:
    """
    Decorator for adding the attribute ``"log_op_type"`` and setting it to
    ``"qec_round"`` to a function.
    """
    if "log_op_type" not in dir(func):
        func.log_op_type = []
    func.log_op_type += ["qec_round"]
    func.rot_basis = None
    if "num_qubits" not in dir(func):
        # avoid overwritting the number of qubits when concatenating decorators
        func.num_qubits = None
    func.noiseless = False
    return func


def sq_gate(func: LogOpCallable) -> LogOpCallable:
    """
    Decorator for adding the attribute ``"log_op_type"`` and setting it to
...
def tq_gate(func: LogOpCallable) -> LogOpCallable:
    """
    Decorator for adding the attribute ``"log_op_type"`` and setting it to
...
def qubit_init_z(func: LogOpCallable) -> LogOpCallable:
    """
...
def qubit_init_x(func: LogOpCallable) -> LogOpCallable:
    """
    Decorator for adding the attribute ``"log_op_type", "rot_basis"`` and setting
    them to ``"qubit_init", False`` (respectively) to a function.
    """
...
def logical_measurement_z(func: LogOpCallable) -> LogOpCallable:
    """
...
def logical_measurement_x(func: LogOpCallable) -> LogOpCallable:
    """
    Decorator for adding the attributes ``"log_op_type", "rot_basis"`` and setting
