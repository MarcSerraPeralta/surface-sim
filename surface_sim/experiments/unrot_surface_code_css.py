...
def repeated_sqrt_x_experiment(
    model: Model,
    layout: Layout,
    detectors: Detectors,
    num_sqrt_x_gates: int,
    num_rounds_per_gate: int,
    data_init: dict[str, int] | list[int],
    rot_basis: bool = False,
    anc_reset: bool = True,
    ...
    """Returns the circuit for running a repeated-SQRT_X experiment.
    ...
    detectors
        Detector definitions to use.
    num_s_gates
        Number of logical (transversal) SQRT_X gates to run in the experiment.
    num_rounds_per_gate
        Number of QEC cycles to be run after each logical SQRT_X gate.
    data_init
        Bitstring for initializing the data qubits.
    rot_basis
        If ``True``, the repeated-SQRT_X experiment is performed in the X basis.
        If ``False``, the repeated-SQRT_X experiment is performed in the Z basis.
        By deafult ``False``.
    anc_reset
        If ``True``, ancillas are reset at the beginning of the QEC cycle.
        By default ``True``.
    ...
    if not isinstance(num_sqrt_x_gates, int):
        raise ValueError(
            f"'num_sqrt_x_gates' expected as int, got {type(num_sqrt_x_gates)} instead."
        )
    if (num_sqrt_x_gates < 0) or (num_sqrt_x_gates % 2 == 1):
        raise ValueError("'num_sqrt_x_gates' needs to be an even positive integer.")
    ...
    for _ in range(num_sqrt_x_gates):
        experiment += log_fold_trans_sqrt_x(model, layout, detectors)
        for _ in range(num_rounds_per_gate):
            experiment += qec_round(model, layout, detectors, anc_reset, anc_detectors)
    experiment += log_meas(
        model, layout, detectors, rot_basis, anc_reset, anc_detectors
